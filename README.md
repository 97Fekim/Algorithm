...
- <code>String[] strs = String.split("__") </code>로 <code>?"__"</code>을 기준으로 쪼개진 문자열 배열을 얻는다.
- <code>int pos = str.indexOf("__")</code> 로, <code>"__"</code> 문자가 있는 인덱스를 pos에 반환받는다. 발견하지 못하면, pos에 -1를 리턴한다.
